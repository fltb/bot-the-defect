...
class Chatter:
    ...
    def _build_chat_messages(self, history_summary, recent_hist, retrieved_ctx, user_msg):
        ...
        system_msg = f"""\
[角色设定]
你扮演{self.bot_role}，玩家扮演{self.user_role}。严格遵守角色设定：
{self.bot_role_info}
...
"""
    ...
    def _create_system_prompt(self) -> str:
        return f"""[角色设定]
{self.bot_role}（你） ↔ {self.user_role}（玩家）
角色特征：{self.bot_role_info}
[输出要求]
• 格式：(动作/神态) 回复内容
• 长度： 1-2 句话
• 风格：保持角色一致性
"""
    ...
    def _create_context_prompt(self, ):
        template_str = """你可以参考以下信息和模仿以下对话来完善你的输出
[参考对话记录和背景信息]
{ context_str }
"""
        # 2. 创建 PromptTemplate（此时 template_vars 会自动绑定所有 {} 变量）
        return template_str
